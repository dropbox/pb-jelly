...
lazy_static! {
  pub static ref Timestamp_default: Timestamp = Timestamp::default();
}
impl ::pb::Message for Timestamp {
  ...
  fn deserialize<B: ::pb::BlobReader>(&mut self, mut buf: &mut B) -> ::std::io::Result<()> {
    while let Some((field_number, typ)) = ::pb::wire_format::read(&mut buf)? {
      match field_number {
        1 => {
          ::pb::ensure_wire_format(typ, ::pb::wire_format::Type::Varint, "Timestamp", 1)?;
          let mut val: i64 = ::std::default::Default::default();
          ::pb::Message::deserialize(&mut val, buf)?;
          self.seconds = val;
        }
        2 => {
          ::pb::ensure_wire_format(typ, ::pb::wire_format::Type::Varint, "Timestamp", 2)?;
...
impl ::pb::MessageDescriptor for Timestamp {
