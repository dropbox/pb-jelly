...
lazy_static! {
  pub static ref FieldMask_default: FieldMask = FieldMask::default();
}
impl ::pb::Message for FieldMask {
  fn compute_size(&self) -> usize  {
    let mut size = 0;
    let mut paths_size = 0;
    for val in &self.paths {
      let l = ::pb::Message::compute_size(val);
      paths_size += ::pb::wire_format::serialized_length(1);
      paths_size += ::pb::varint::serialized_length(l as u64);
      paths_size += l;
    }
    size += paths_size;
    size
  ...
  fn deserialize<B: ::pb::BlobReader>(&mut self, mut buf: &mut B) -> ::std::io::Result<()> {
    while let Some((field_number, typ)) = ::pb::wire_format::read(&mut buf)? {
      match field_number {
        1 => {
          ::pb::ensure_wire_format(typ, ::pb::wire_format::Type::LengthDelimited, "FieldMask", 1)?;
...
impl ::pb::MessageDescriptor for FieldMask {
